...
try {
    ...
    // Базовый запрос
    $sql = "SELECT * FROM product WHERE gender = 'жен'";
    $params = [];
    ...
    // Получаем женские товары с учетом фильтров
    $stmt = $pdo->prepare($sql);
    $stmt->execute($params);
    $womenProduct = $stmt->fetchAll(PDO::FETCH_ASSOC);

    // Получаем уникальные размеры для фильтра
    $stmt = $pdo->query("SELECT DISTINCT size FROM product WHERE gender = 'жен' ORDER BY size");
        ...
        <div class="filters">
            <form method="get" id="filter-form">
                ...
                <?php if (isset($_GET['min_price']) || isset($_GET['max_price']) || isset($_GET['size'])): ?>
                    <a href="women.php" class="clear-filters">Сбросить фильтры</a>
                <?php endif; ?>
            </form>
        </div>

        <div class="product-grid">
            <?php foreach ($womenProduct as $product): 
