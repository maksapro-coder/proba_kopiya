...
try {
    ...
    // Базовый запрос
    $sql = "SELECT * FROM product WHERE gender = 'муж'";
    $params = [];
    ...
    // Получаем мужские товары с учетом фильтров
    $stmt = $pdo->prepare($sql);
    $stmt->execute($params);
    $menProduct = $stmt->fetchAll(PDO::FETCH_ASSOC);

    // Получаем уникальные размеры для фильтра
    $stmt = $pdo->query("SELECT DISTINCT size FROM product WHERE gender = 'муж' ORDER BY size");
        ...
        <div class="filters">
            <form method="get" id="filter-form">
                ...
                <?php if (isset($_GET['min_price']) || isset($_GET['max_price']) || isset($_GET['size'])): ?>
                    <a href="men.php" class="clear-filters">Сбросить фильтры</a>
                <?php endif; ?>
            </form>
        </div>

        <div class="product-grid">
            <?php foreach ($menProduct as $product): 
